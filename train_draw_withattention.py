...
from DRAWnet_withattention import VAE_bernoulli_attention
import numpy as np
...
def parse_args():
    ...
    p.add_argument('-Pr', '--Read_patch',default=vae_Readpatch_def, metavar='INT',type=int,
        help='Read attention window size (default: %d)' % vae_Readpatch_def)
    p.add_argument('-Pw', '--Write_patch',default=vae_Writepatch_def, metavar='INT',type=int,
        help='Write attention window size (default: %d)' % vae_Writepatch_def)
    ...
    try:
        ...
        if (args.Read_patch < 1): raise ValueError('you must set --vae_enc >= 1')
        if (args.Write_patch < 1): raise ValueError('you must set --vae_dec >= 1')
